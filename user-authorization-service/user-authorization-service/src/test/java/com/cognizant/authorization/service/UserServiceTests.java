...
class UserServiceTests {
	...
	void testLoadUserByUsername_validUsername() {
		...
		// Data to mock
		/*UserRegistrationDetails user = new UserRegistrationDetails("sonia", "sonia", "ADMIN");

		// Convert to optional
		Optional<UserLoginDetails> userOptional = Optional.of(user);

		// user-name to check - correct credentials
		final String id = "sonia";
		...
		// Mock the repository
		when(userLoginRepository.findById(id)).thenReturn(userOptional);
	...
	void testLoadUserByUsername_invalidUsername() {
		...
		UserLoginDetails user = new UserLoginDetails("incorrect", "incorrect", "ADMIN");

		// Data to mock
		Optional<UserLoginDetails> userOptional = Optional.empty();

		// user-name to check - incorrect credentials
		final String id = "incorrect";
